...
class TestDataCleaner(unittest.TestCase):
    ...
    def test_drop_invalid_rows_raises_keyerror_for_unknown_column(self):
        ...
        """
        df_original2 = make_sample_df()
        cleaner = DataCleaner()

        with self.assertRaises(KeyError):
            cleaner.drop_invalid_rows(df_original2, cols=["does_not_exist"])
    ...
    def test_trim_strings_strips_whitespace_without_changing_other_columns(self):
        ...
        """
        df_original3 = make_sample_df()
        df_original3["name"] = df_original3["name"].astype("string")
        cleaner = DataCleaner()

        # Guardar copia del original para comparar después
        df_copia = df_original3.copy(deep=True)

        # Aplicar trim_strings solo a la columna "name"
        df_resultante = cleaner.trim_strings(df_original3, cols=["name"])

        # Verificar que el DataFrame original no fue modificado
        self.assertEqual(df_original3.loc[0, "name"], df_copia.loc[0, "name"])
        self.assertEqual(df_original3.loc[1, "name"], df_copia.loc[1, "name"])

        # Verificar que en el DataFrame resultante los valores de "name" no tienen espacios
        self.assertEqual(df_resultante.loc[0, "name"], df_original3.loc[0, "name"].strip())
        self.assertEqual(df_resultante.loc[1, "name"], df_original3.loc[1, "name"].strip())

        # Verificar que la columna "city" no fue modificada
        assert_series_equal(df_resultante["city"], df_original3["city"])
    ...
    def test_remove_outliers_iqr_removes_extreme_values(self):
        ...
        df_resultante = cleaner.remove_outliers_iqr(df6, col="age", factor=1.5)

        # Verificar que el valor extremo fue eliminado
        self.assertNotIn(120, df_resultante["age"].values)

        # Verificar que al menos uno de los valores no extremos permanece
        self.assertTrue(any(val in df_resultante["age"].values for val in [25, 35]))
    ...
    def test_remove_outliers_iqr_raises_keyerror_for_missing_column(self):
        ...
        with self.assertRaises(KeyError):
            cleaner.remove_outliers_iqr(df7, col="salary", factor=1.5)
    ...
    def test_remove_outliers_iqr_raises_typeerror_for_non_numeric_column(self):
        ...
        with self.assertRaises(TypeError):
            cleaner.remove_outliers_iqr(df8, col="city", factor=1.5)
