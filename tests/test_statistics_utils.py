...
class TestStatisticsUtils(unittest.TestCase):
    ...
    def test_zscore_has_mean_zero_and_unit_std(self):
        ...
        """
        numeros4 = np.array([10, 20, 30, 40])
        utils = StatisticsUtils()
        resultado = utils.zscore(numeros4)
        # verificar qque la media = 0
        self.assertAlmostEqual(np.mean(resultado), 0.0, places=7)
        # verificar que la des std = 1
    ...
    def test_zscore_raises_for_zero_std(self):
        ...
        """
        numeros5 = np.array([5, 5, 5])
        utils = StatisticsUtils()

        # se lanza un ValueError
        with self.assertRaises(ValueError):
            utils.zscore(numeros5)
    ...
    def test_min_max_scale_maps_to_zero_one_range(self):
        ...
        """
        numeros6 = np.array([2, 4, 6])
        utils = StatisticsUtils()
        # llaamar a min.max.scale
        resultado5 = utils.min_max_scale(numeros6)
        # Verificar que el valor mínimo del resultado es 0.0
        self.assertAlmostEqual(np.min(resultado5), 0.0, places=7)
        # Verificar que el valor máximo del resultado es 1.0
        self.assertAlmostEqual(np.max(resultado5), 1.0, places=7)
        # verificar que los valores son correctos 
        val_esperado = np.array([0.0, 0.5, 1.0])
        npt.assert_allclose(resultado5, val_esperado, rtol=1e-7, atol=1e-7)
    ...
    def test_min_max_scale_raises_for_constant_values(self):
        ...
        """
        numeros7 = np.array([3, 3, 3])
        utils = StatisticsUtils()
        # verificar error
        with self.assertRaises(ValueError):
            utils.min_max_scale(numeros7)
